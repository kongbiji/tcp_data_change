...
static bool check_exist_flow = false;
...
bool check_flow(unsigned char * data, int size){
    ...
    iter = m.begin();
    if ( ((iter = m.find(key)) != m.end()) && (iter->second != 0)){
        printf("*** same flow detected !! ***\n");
        packet->tcp.seq = htonl(ntohl(packet->tcp.seq) + iter->second);
        ret = true;
    }
    iter = m.begin();
    if ( ((iter = m.find(key2)) != m.end()) && (iter->second != 0)){
        printf("*** reverse flow detected !! ***\n");
...
void data_change(unsigned char * data, int size){
    ...
    if(size > (ihl + tcp_l)){ // if HTTP data is exist
        ...
        if(check_data_changed == true){
            if(distance > 0){ //change content length 
                string cont_len = "Content-Length: "; 
                pos = tmp_str.find(cont_len) + cont_len.length();
                int changed_len;
                if(times > 0){
            ...
            if(times > 0){
                size += (distance * times);
                packet->ip.total_len = htons(ntohs(packet->ip.total_len) + (distance * times));
            }else{
                size += distance;
    ...
    if (check_data_changed == true || check_exist_flow == true){
        ...
        memcpy((char *)pseudo_data, ph, sizeof(pseudo_header));
        memcpy((char *)(pseudo_data) + sizeof(pseudo_header), data + ihl, tcp_l);
        memcpy((char *)(pseudo_data) + sizeof(pseudo_header) + tcp_l, data + ihl + tcp_l, size-ihl-tcp_l);
...
static u_int32_t print_pkt (struct nfq_data *tb){
	...
	ret = nfq_get_payload(tb, &data); //data = packet
    	...
    	check_exist_flow = check_flow(data, ret);
...
static int cb(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg, struct nfq_data *nfa, void *data){
    ...
    check_exist_flow = false;
	u_int32_t id = print_pkt(nfa);
	printf("entering callback\n"); 

    if((check_exist_flow == true) || (check_data_changed == true)){
